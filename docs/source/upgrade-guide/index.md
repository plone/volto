...
```
`pnpm` has been updated to version 10 in the project boilerplate generated by Cookieplone.
This change improves performance and compatibility with the latest package management features.
`pnpm` 10 no longer runs lifecycle scripts.
If you have packages that use lifecycle scripts (such as `preinstall` or `postinstall`) in {file}`package.json`, you must configure `pnpm`'s [`onlyBuiltDependencies` setting](https://pnpm.io/10.x/settings#onlybuiltdependencies) to allow them.
...
index c469fe9..c39a324 100644
--- a/frontend/Dockerfile
+++ b/frontend/Dockerfile
@@ -5,14 +5,12 @@ FROM plone/frontend-builder:${VOLTO_VERSION} AS builder
...
async construction before the test is fired. See this CodeSandbox example:

https://codesandbox.io/p/sandbox/loadable-async-tests-l2bx9?file=%2Fsrc%2FApp.js
