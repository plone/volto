...
If you have changed any dependency, run `make install` from the repository root and commit the updated {file}`pnpm-lock.yaml` first; otherwise acceptance tests may fail due to an out-of-date lockfile.
