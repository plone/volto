...
myst:
  html_meta:
    ...
    "property=og:title": "Type custom content types"
    "keywords": "Seven, TypeScript, content types, @plone/types, ContentTypeMap, type narrowing"
---

# Type custom content types

Most Plone projects add content types beyond the standard ones that ship with the CMS.
...
When you add your type to it, TypeScript automatically narrows the `Content` union when you check the `@type` field - no manual `as` casts required.


## Define the content type interface

Create a `.d.ts` file for your type and extend `ContentBase` from `@plone/types`.
Set the `@type` field to the **exact string** that Plone uses for that content type, which you can verify from the `@type` key in any REST API response for that content type.
This is, for example, `Event` for the Event content type or `Document` for the Page content type.

```{code-block} ts
:caption: packages/\<add-on-name>/types/content.d.ts
...
`Content` is a discriminated union of all registered types — extending a union type is not valid TypeScript.
...
In the same `.d.ts` file, add a `declare module` block to merge your interface into the `ContentTypeMap` registry.
...
TypeScript picks up the augmentation automatically whenever the `.d.ts` file is included in the compilation.
...
The file must be a **declaration file** (`.d.ts`), not a plain TypeScript file (`.ts`).
A `declare module` augmentation inside a `.ts` file is treated as a local module augmentation and will not apply globally.

Make sure the file is included in your TypeScript compilation.
If you use a `tsconfig.json` with an explicit `include` list, add `types/**/*.d.ts` to it.
...
If you are building an add-on package that others will install, include the type definitions in your published package so that consumers get narrowing automatically.

Export the `.d.ts` file from your package by listing it in your `package.json` `exports` or `types` field, or by placing it in a location covered by your package's `files` list.
...
Alternatively, if the augmentation lives in a side-effect-only `.d.ts` file, by adding a triple-slash reference.
