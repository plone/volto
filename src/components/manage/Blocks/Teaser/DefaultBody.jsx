...
const messages = defineMessages({
  ...
  PlaceholderDescription: {
    id: 'Lorem ipsum dolor sit amet, consetetur sadipscing elitr, sed diam nonumy eirmod tempor invidunt ut labore et dolore magna aliquyam erat, sed diam voluptua. At vero eos et accusam et justo duo dolores et ea rebum.',
    defaultMessage: 'Lorem ipsum dolor sit amet, consetetur sadipscing elitr, sed diam nonumy eirmod tempor invidunt ut labore et dolore magna aliquyam erat, sed diam voluptua. At vero eos et accusam et justo duo dolores et ea rebum.',
  },
});
...
const TeaserDefaultTemplate = (props) => {
  ...
  useEffect(() => {
    if (href && alwaysLive) {
      dispatch(getContent(flattenToAppURL(href['@id']), null, id)).then((resp) => {
        if (resp) {
  ...
  return isLive && alwaysLive ? (
    <div className={cx('block teaser', className)}>
      <>
        {!liveData.current && isEditMode && (
          <div className="teaser-item default">
            ...
            <div className="content">
              <div className="headline">{intl.formatMessage(messages.PlaceholderHeadtitle)}</div>
        ...
        {liveData.current && (
          <MaybeWrap condition={!isEditMode} as={UniversalLink} href={liveData.current['@id']} target={data?.openLinkInNewTab ? '_blank' : null}>
            <div className="teaser-item default">
              {(liveData.current.hasPreviewImage || liveData.current.image_field || image) && (
                <div className="image-wrapper">
                  <Image src={liveData.current?.preview_image ? flattenToAppURL(liveData.current.preview_image.scales.teaser.download) : liveData.current?.image ? flattenToAppURL(liveData.current.image.scales.teaser.download) : placeholderIMG} alt="" loading="lazy" />
                </div>
              )}
              <div className="content">
                {liveData.current?.head_title && <div className="headline">{liveData.current.head_title}</div>}
                <h2>{liveData.current?.title}</h2>
                {!liveData.current.hide_description && <p>{liveData.current?.description}</p>}
              </div>
            </div>
          </MaybeWrap>
        )}
