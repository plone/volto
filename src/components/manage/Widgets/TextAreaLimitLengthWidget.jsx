...
class TextAreaLimitLengthWidget extends Component {
  ...
  onHandleChange = (id, value) => {
    ...
    if (remlength < 0) {
      this.setState({
        lengthError: `you have exceed word limit by ${remlength}`,
      });
    } else {
      this.setState({
        lengthError: '',
          ...
          <Grid.Row stretched>
            ...
            <Grid.Column width="8">
              ...
              {lengthError.length > 0 && (
                <Label key={lengthError} basic color="red" pointin>
                  {lengthError}
                </Label>
              )}
              {error.length > 0 && (
 ...
 * @static
 */
TextAreaLimitLengthWidget.propTypes = {
 ...
 * @static
 */
TextAreaLimitLengthWidget.defaultProps = {
  description: null,
  required: false,
  error: [],
  value: null,
...
export default injectIntl(TextAreaLimitLengthWidget);
