...
const safe_config = (func) => (config) => {
  const res = func(config);
  if (typeof res === 'undefined') {
    console.error("Configuration function doesn't return config", func);
...
const load = (config) => {
  ...
  return addonLoaders.reduce((acc, apply) => safe_config(apply(acc)), config);
};
export default load;
`;
