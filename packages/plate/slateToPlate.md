...
This package includes small migration helpers to make legacy Slate content render correctly in Plate without pre-processing. They run both in SSR (static pass) and in the client (plugin normalizers).
...
`normalizeLegacyValue(value, linkType?)` runs bold, italic, strikethrough, list, and link static migrations in place. Use this before `usePlateEditor` on the server, or in a one-off migration script.
