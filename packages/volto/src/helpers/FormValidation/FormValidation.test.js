...
describe('FormValidation', () => {
  ...
  describe('validateFieldsPerFieldset', () => {
    ...
    it('custom-validator-widget - validates and stores errors in internalErrors - Fails', () => {
      let newSchema = {
        properties: {
          ...schema.properties,
          objectList: {
            title: 'Object List',
            description: 'A list of complex items',
            widget: 'object_list',
          },
        },
        required: [],
      ...
      config.registerUtility({
        ...
        method: ({ value, field, formatMessage }) => {
          // Simulating an error in specific list items
          return [
            {
              id: 'item-1',
              field: 'name',
              message: 'Invalid name in item 1',
            },
            {
              id: 'item-2',
              field: 'email',
              message: 'Invalid email in item 2',
            },
          ];
        },
      });

      const resultado = FormValidation.validateFieldsPerFieldset({
        schema: newSchema,
        formData: {
          username: 'test username',
          objectList: [
            { '@id': 'item-1', name: '', email: 'test@example.com' },
            { '@id': 'item-2', name: 'teste', email: 'invalid' },
          ],
        },
        formatMessage,
      });

      // Checking if the objectList field has errors and internalErrors
      expect(resultado).toHaveProperty('objectList');
      expect(resultado.objectList).toHaveProperty('internalErrors');
    });

    it('custom-validator-widget - validates and stores no errors when valid - Succeeds', () => {
      let newSchema = {
        properties: {
          ...schema.properties,
          objectList: {
      ...
      const result = FormValidation.validateFieldsPerFieldset({
        schema: newSchema,
        formData: {
          username: 'test username',
          objectList: [
            { '@id': 'item-1', name: 'Valid name', email: 'test@example.com' },
          ],
        },
        formatMessage,
      });

      // Checking that there are no errors for the objectList field
      expect(result).not.toHaveProperty('objectList');
    });
  });
});
