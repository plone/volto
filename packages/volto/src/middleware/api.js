    ...
    if (socket) {
      ...
      actionPromise.then(
        ...
        (error) => {
          ...
          // doesn't hide the SSR content. Suppress failures for all actions
          // except user-initiated actions (LOGIN, etc.) which must always
          // dispatch _FAIL so loaders stop and errors surface.
          const shouldIgnoreHydrationError =
            isHydrating && !hasExistingError && type !== LOGIN;
