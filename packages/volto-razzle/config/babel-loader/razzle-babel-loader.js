...
const razzleBabelPreset = require('@volto/babel-preset-razzle');
...
module.exports = babelLoader.custom(function (babel) {
  ...
  const applyCommonJs = babel.createConfigItem(
    require('@volto/babel-preset-razzle/babel-plugins/commonjs'),
    { type: 'plugin' },
  );
  ...
  return {
    ...
    config: function (cfg, cfgOpts) {
      ...
      if (cfg.hasFilesystemConfig()) {
        for (const file of [cfg.babelrc, cfg.config]) {
          ...
          if (
            file &&
            verbose &&
            !configs.has(`${file}.${isServer ? 'node' : 'web'}`)
          ) {
        ...
        presetOptions.presets = (presetOptions.presets || []).concat([
          presetItem,
        ]);
      }
      ...
      if (shouldUseReactRefresh) {
        ...
        if (!isServer) {
          const noAnonymousDefaultExportPlugin = babel.createConfigItem(
            [
              require('@volto/babel-preset-razzle/babel-plugins/no-anonymous-default-export'),
              {},
            ],
            { type: 'plugin' },
          );
      ...
      if (isModern) {
        ...
        presetOptions.presets = (additionalPresets || []).concat([
          presetItemModern,
        ]);
      }
