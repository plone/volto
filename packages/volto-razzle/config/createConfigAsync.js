...
module.exports = (
  ...
  return new Promise(async (resolve) => {
    ...
    const shouldDisableWebpackbar =
      razzleOptions.disableWebpackbar === true ||
      razzleOptions.disableWebpackbar === target;

    let webpackOptions = {};

    const hasPublicDir = fs.existsSync(paths.appPublic);

    const hasStaticExportJs =
      fs.existsSync(paths.appStaticExportJs + '.js') ||
    ...
    const dotenv = getClientEnv(
      ...
      IS_DEV,
      {
        ...
        forceRuntimeEnvVars: razzleOptions.forceRuntimeEnvVars,
        webpackObject,
      },
      paths,
    );
      ...
      use: [
        {
          ...
          options: {
            ...
            configFile: razzleOptions.enableTargetBabelrc
              ? path.resolve(paths.appPath, `.babelrc.${target}`)
              : undefined,
            hasModern: false,
            development: IS_DEV,
    ...
    const nodeExternalsFunc = (context, request, callback) => {
      if (
        webpackOptions.notNodeExternalResMatch &&
        webpackOptions.notNodeExternalResMatch(request, context)
      ) {
        if (debugNodeExternals) {
          console.log(
            `Not externalizing ${request} (using notNodeExternalResMatch)`,
          );
        }
        return callback();
      }
    ...
    const postCssOptions = hasPostCssConfig
      ? undefined
      : { postcssOptions: webpackOptions.postCssOptions };
      ...
      module: {
        ...
        rules: [
          webpackOptions.babelRule,
          {
            ...
            use:
              clientOnly || webpackOptions.enableHtmlWebpackPlugin
                ? (info) => {
    ...
    if (IS_NODE) {
      ...
      config.externals = (!IS_SERVERLESS ? [nodeExternalsFunc] : []).concat(
        nodeExternals,
      );
      ...
      config.plugins = [
        ...
        new ManifestPlugin({
          ...
          generate: (seed, files) => {
            ...
            files.forEach((file) => {
              if (file.isChunk) {
                const groups = ((file.chunk || {})._groups || []).forEach(
